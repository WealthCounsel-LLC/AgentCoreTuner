...
/// Lists sessions for an actor in a specific memory pool.
pub fn list_sessions(
    ...
    list_sessions_with(&RealCli, profile, region, memory_id, actor_id)
}
...
pub fn list_sessions_with(
    ...
    let stdout = check_output(&out, "bedrock-agentcore list-sessions")?;
    parse_list_sessions_response(&stdout)
}

fn parse_list_sessions_response(json: &str) -> Result<Vec<crate::types::SessionSummary>, String> {
    let v: serde_json::Value = serde_json::from_str(json)
        .map_err(|e| format!("Failed to parse list-sessions response: {e}"))?;
    ...
    for item in summaries {
        ...
        if !session_id.is_empty() {
            sessions.push(crate::types::SessionSummary {
                session_id,
                actor_id,
                created_at,
            });
