...
/// Compute content hash for a recipe (based on configuration fields, not metadata).
pub fn compute_recipe_hash(recipe: &RecipeEntry) -> String {
    let mut hasher = DefaultHasher::new();
    recipe.model_id.hash(&mut hasher);
    recipe.system_prompt.hash(&mut hasher);
    recipe.prompt_id.hash(&mut hasher);
    recipe.memory_id.hash(&mut hasher);
    recipe.namespace_pattern.hash(&mut hasher);
    recipe.qualifier.hash(&mut hasher);
    // Hash extra_params as JSON string for consistency
    if let Ok(json) = serde_json::to_string(&recipe.extra_params) {
        json.hash(&mut hasher);
    }
    format!("{:016x}", hasher.finish())
}

/// Compute content hash for a prompt (based on content field).
pub fn compute_prompt_hash(prompt: &PromptEntry) -> String {
    let mut hasher = DefaultHasher::new();
    prompt.content.hash(&mut hasher);
    format!("{:016x}", hasher.finish())
}

/// Check if a recipe with the given hash exists locally.
pub fn recipe_hash_exists(hash: &str) -> bool {
    if hash.is_empty() {
        return false;
    }
    match list_entries() {
        Ok(entries) => entries.iter().any(|e| e.content_hash == hash),
        Err(_) => false,
    }
}

/// Check if a prompt with the given hash exists locally.
pub fn prompt_hash_exists(hash: &str) -> bool {
    if hash.is_empty() {
        return false;
    }
    match list_prompts() {
