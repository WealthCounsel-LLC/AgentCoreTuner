...
const DeleteRecipeDialog: Component<DeleteRecipeDialogProps> = (props) => {
  const handleDelete = () => {
    if (props.recipe) {
      ...
      <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
        <div class="bg-surface rounded-standard border border-border w-full max-w-md">
          <div class="flex items-center justify-between p-4 border-b border-border">
            <h2 class="text-lg font-semibold text-error">Delete Snapshot</h2>
            <button
          ...
          <div class="p-4">
            <div class="flex items-start gap-3">
              ...
              <div>
                <p class="text-sm">
                  Are you sure you want to delete <strong>{props.recipe?.name}</strong>?
                </p>
                <p class="text-sm text-text-dim mt-2">
                  This action cannot be undone. The snapshot and all its version history will be permanently removed.
                </p>
              </div>
            </div>
          </div>
