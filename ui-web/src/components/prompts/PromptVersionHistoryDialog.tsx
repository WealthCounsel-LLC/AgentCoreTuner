...
const PromptVersionHistoryDialog: Component<PromptVersionHistoryDialogProps> = (props) => {
  const [history, setHistory] = createSignal<string[]>([]);
  const [loading, setLoading] = createSignal(false);
  const [error, setError] = createSignal<string | null>(null);

  createEffect(async () => {
    if (props.isOpen && props.promptId) {
      setLoading(true);
      setError(null);
      try {
        const commits = await tauri.promptHistory(props.promptId);
        setHistory(commits);
      } catch (err) {
        setError(String(err));
        setHistory([]);
        ...
        <div class="bg-surface rounded-standard border border-border w-full max-w-lg max-h-[80vh] flex flex-col">
          <div class="flex items-center justify-between p-4 border-b border-border">
            <div>
              ...
              <p class="text-sm text-text-dim">{props.promptName}</p>
            </div>
            <button
...
export default PromptVersionHistoryDialog;
